<template>
  <div class="h-full flex-1 flex-col rounded border border-border bg-bg p-5 md:flex md:overflow-auto">
    <!-- 表格工具 -->
    <div class="mb-2.5 flex flex-col justify-between gap-y-2.5 md:flex-row">
      <!-- 左侧工具 -->
      <div class="toolbar-left flex flex-wrap gap-x-2 gap-y-2.5 md:gap-x-3">
      ...
      <!-- 右侧工具 -->
      <div class="toolbar-right flex flex-wrap gap-x-2 gap-y-2.5 md:gap-x-3">
          ...
          <template #default="scope">
            ...
            <template v-else-if="col.templet === 'switch'">
              <template v-if="col.prop">
                ...
                <el-switch
                  ...
                  @change="pageData.length > 0 && handleModify(col.prop, scope.row[col.prop], scope.row)"
                />
              </template>
            </template>
            <!-- 生成输入框组 -->
            ...
            <template v-else-if="col.templet === 'icon'">
              <template v-if="col.prop">
                ...
                <template v-else>
                  <component :is="getSvgIcon(scope.row[col.prop])" class="h-6 w-6" />
                </template>
              </template>
            </template>
            <!-- 格式化时间 -->
            <template v-else-if="col.templet === 'date'">
              <template v-if="col.prop">
                {{
                  scope.row[col.prop]
                    ? useDateFormat(scope.row[col.prop], col.dateFormat ?? 'YYYY-MM-DD HH:mm:ss').value
                    : ''
                }}
              </template>
            </template>
    ...
    <div v-if="showPagination" class="mt-4">
      <el-scrollbar :class="['h-8!', { 'justify-end': contentConfig?.pagePosition === 'right' }]">
