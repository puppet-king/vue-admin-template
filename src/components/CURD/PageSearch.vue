        ...
        <el-form-item :class="{ 'col-[auto/-1] justify-self-end': searchConfig?.grid === 'right' }">
          ...
          <template v-if="isExpandable && formItems.length > showNumber">
            <el-link class="ml-3" type="primary" underline="never" @click="isExpand = !isExpand">
              {{ isExpand ? '收起' : '展开' }}
              <component :is="isExpand ? ChevronUp : ChevronDown" class="ml-2 h-4 w-4" />
            </el-link>
          </template>
        </el-form-item>
      </el-form>
...
import { ChevronUp, ChevronDown, Search, RotateCcw } from '@lucide/vue'
...
const showNumber = computed(() => (isExpandable.value ? (props.searchConfig?.showNumber ?? 3) : formItems.length))
