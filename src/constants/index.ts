 ...
 * 静态资源 CDN 域名基础前缀
 */
export const STATIC_RESOURCE_BASE_URL = 'https://static.juqingsong.cn'

/**
 * 获取完整静态资源 URL (若为相对路径则自动拼接静态资源 CDN 域名)
 * @param path 资源相对路径或绝对链接
 */
export function getStaticResourceUrl(path?: string): string {
  if (!path) return ''
