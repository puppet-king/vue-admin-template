...
const MenuIcon = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      if (!props.icon) {
        return h(ElIcon, null, () => h('div', { style: 'width: 1em; height: 1em' }))
      }
      ...
      if (typeof resolved !== 'string') {
        return h(ElIcon, null, () => h(resolved))
      }
...
const { showTagsView, showLogo, isSidebarOpen, toggleSidebar, sideMenuRoutes, activeTopMenuPath } = useLayout()
...
const useMenuColors = computed(
  () =>
    settingsStore.resolvedTheme === ThemeMode.DARK || settingsStore.sidebarColorScheme === SidebarColor.CLASSIC_BLUE,
)
...
function navigateToFirstMenu(menus: RouteRecordRaw[]) {
  ...
  if (first.children?.length) {
    ...
    router.push({
      name: first.name,
      query: typeof first.meta?.params === 'object' ? (first.meta.params as LocationQueryRaw) : undefined,
    })
  }
}
